let recognizer;var start=!1;function predictWord(){const words=recognizer.wordLabels();recognizer.listen(({scores})=>{scores=Array.from(scores).map((s,i)=>({score:s,word:words[i]}));scores.sort((s1,s2)=>s2.score-s1.score)},{probabilityThreshold:0.5})}
...
function listen(){level++;var demo=document.querySelector('#demo');demo.style.visibility='hidden';reset=!0;setInterval(draw1,10);if(recognizer.isListening()){recognizer.stopListening();toggleButtons(!0);document.getElementById('listen').textContent='Start';return}
...
app();var reset=!1;var context;var enemy;var dx=2;var dy=1;var y=100;var x=150;var x0=20;var dx0=1;var y0;var level=0;function draw1(){if(level>=5){toggleButtons(!1);document.querySelector('#console').textContent=`Level ${level} / 5. Refresh page to restart.`}
if(reset===!0){y=100;x=150;x0=20;reset=!1}
if(Math.abs(x-x0)<7.5&&Math.abs(y-y0)<2&&level<5){document.querySelector('#console').textContent=`Level ${level} / 5`;return}
context=myCanvas.getContext('2d');context.clearRect(0,0,300,300);context.beginPath();context.fillStyle="green"
context.arc(x,y,5,0,Math.PI*2,!0);context.closePath();context.fill();y0=y+(Math.random()*Math.floor(4));enemy=myCanvas.getContext('2d');enemy.beginPath();enemy.fillStyle="red"
enemy.arc(x0,y0,5,0,Math.PI*2,!0);enemy.closePath();enemy.fill();if(x<10||x>290){dx=-dx}
if(x0<10||x0>290){dx0=-dx0}
if(y<20||y>150){dy=-dy}
if(right===!0){x+=dx}
if(left===!0){x-=dx}
y+=dy;x0+=dx0}